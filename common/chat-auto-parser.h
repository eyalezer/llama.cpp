...
struct autoparser {
    ...
    // Find the starting marker for the system message and tool-response message
    // (needed so callers, e.g. --triattention-protect-roles, can locate & protect these spans)
    std::string detect_system_start_marker(const common_chat_template & tmpl);
    std::string detect_tool_start_marker(const common_chat_template & tmpl);
