...
// Dispatch a turbo vector case unconditionally. Turbo instances are always compiled (see the
// dedicated turbo instance glob in ggml-cuda/CMakeLists.txt), so there is no GGML_CUDA_FA gating
// for them. Head-dim geometry for turbo is validated separately in ggml_cuda_get_best_fattn_kernel.
#define FATTN_VEC_CASE_DIRECT(D, type_K_case, type_V_case)                                                   \
    if (head_size == (D) && type_K == GGML_TYPE_##type_K_case && type_V == GGML_TYPE_##type_V_case) {        \
        return ggml_cuda_flash_attn_ext_vec_case<D, GGML_TYPE_##type_K_case, GGML_TYPE_##type_V_case>;       \
    }

#define FATTN_VEC_CASES_DIRECT_ALL_D(type_K_case, type_V_case) \
    FATTN_VEC_CASE_DIRECT( 64, type_K_case, type_V_case)       \
    FATTN_VEC_CASE_DIRECT(128, type_K_case, type_V_case)       \
    FATTN_VEC_CASE_DIRECT(256, type_K_case, type_V_case)
...
static fattn_vec_case_t ggml_cuda_get_fattn_vec_case(const int64_t head_size, const ggml_type type_K, const ggml_type type_V) {
    ...
    // TurboQuant3 KV cache types (always enabled, always compiled)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO3_0, TURBO3_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO3_0, Q8_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(Q8_0, TURBO3_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(F16,  TURBO3_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO3_0, F16)
    // TurboQuant2 KV cache types (always enabled, always compiled)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO2_0, TURBO2_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO2_0, Q8_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(Q8_0, TURBO2_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(F16,  TURBO2_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO2_0, F16)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO3_0, TURBO2_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO2_0, TURBO3_0)
    // TurboQuant4 KV cache types (always enabled, always compiled)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO4_0, TURBO4_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO4_0, Q8_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(Q8_0, TURBO4_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(F16,  TURBO4_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO4_0, F16)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO4_0, TURBO3_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO3_0, TURBO4_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO4_0, TURBO2_0)
    FATTN_VEC_CASES_DIRECT_ALL_D(TURBO2_0, TURBO4_0)
    GGML_ABORT("fatal error");
    return nullptr;
}
...
static void ggml_cuda_flash_attn_ext_vec(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    fattn_vec_case_t vec_case = ggml_cuda_get_fatn_vec_case(Q->ne[0], K->type, V->type);
    if (vec_case == nullptr) {
        ...
        vec_case = ggml_cuda_get_fatn_vec_case(Q->ne[0], GGML_TYPE_F16, GGML_TYPE_F16);
    }
...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor * dst) {
    ...
    // Allow mixed KV types only for combinations that have FA template instances compiled in:
    // - turbo2/3/4 + q8_0 (turbo cache work)
    // - f16/bf16 + q8_0 (common K=f16, V=q8_0 setup)
    // Turbo instances are always compiled (see ggml-cuda/CMakeLists.txt), so these are the only
    // mixed combos we can route through the vector/MMA kernels.
    if (K->type != V->type) {
