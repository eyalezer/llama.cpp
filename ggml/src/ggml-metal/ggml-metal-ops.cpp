...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    } else if (ggml_metal_op_mul_mat_use_mm(op, props_dev->has_simdgroup_mm) && (op->src[0]->type == GGML_TYPE_TQ3_1S || op->src[0]->type == GGML_TYPE_TQ4_1S)) {
...
int ggml_metal_op_mul_mat_id(ggml_metal_op_t ctx, int idx) {
    ...
    if (ggml_metal_op_mul_mat_id_use_mm(op, props_dev->has_simdgroup_mm) && (ne21 >= ne21_mm_id_min)) {
...
static bool ggml_metal_op_flash_attn_ext_use_kv_f16(const ggml_tensor * op) {
    ...
    // turbo V has no kf16_vturbo kernel, only kq8_0_vturbo / kturbo* variants
    // that keep K in its raw quantized form. do not dequant K to f16 when V is turbo.
