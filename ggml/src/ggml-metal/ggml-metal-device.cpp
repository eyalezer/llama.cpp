...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext(
    ...
    if (strcmp(type_k, type_v) == 0 && strncmp(type_k, "turbo", 5) != 0) {
        snprintf(base, 256, "kernel_flash_attn_ext_%s_dk%d_dv%d", type_k, dk, dv);
    } else {
        snprintf(base, 256, "kernel_flash_attn_ext_k%s_v%s_dk%d_dv%d", type_k, type_v, dk, dv);
    }
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext_vec(
    ...
    // when KV is dequantized to f16 before flash attention, the pipeline runs
    // the f16 kernels; otherwise use the quantized K and V type names.
    const char * type_k = use_kv_f16 ? "f16" : ggml_type_name(op->src[1]->type);
    const char * type_v = use_kv_f16 ? "f16" : ggml_type_name(op->src[2]->type);
    ...
    // Turbo K/V blocks only support the baseline (Q,NE); the tuned (Q,NE) kernels
    // for these block types are not instantiated, so force baseline NE to avoid a
    // kernel-not-found lookup. applies when either side is turbo (e.g. q8_0 K /
    // turbo V), since those kernels only exist at the baseline NE.
    const bool is_turbo_k = strncmp(type_k, "turbo", 5) == 0;
    const bool is_turbo_v = strncmp(type_v, "turbo", 5) == 0;
    char qne_suffix[16] = {0};
    if (!is_turbo_k && !is_turbo_v && !(nqpsg == 1 && ne == ggml_metal_tuning::fa_vec_baseline_ne(dk, dv))) {
