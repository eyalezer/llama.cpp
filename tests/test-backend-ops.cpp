...
// Test SET_ROWS with TQ3_1S/TQ4_1S destination (weight quantization), then dequantize and compare.
// Validates: f32 -> WHT forward -> centroid quantize -> bit pack -> SET_ROWS
...
struct test_set_rows_tq4_1s : public test_case {
    ...
    std::string vars() override {
        return VARS_TO_STR4(quant_type, ne0, ne1, r) + ",idx=" + ggml_type_name(type_idx);
    }

    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return std::string("SET_ROWS_") + ggml_type_name(quant_type);
    }

    test_set_rows_tq4_1s(ggml_type type_idx = GGML_TYPE_I32,
            int64_t ne0 = 32, int64_t ne1 = 8, int r = 4, ggml_type quant_type = GGML_TYPE_TQ4_1S)
        : quant_type(quant_type), type_idx(type_idx), ne0(ne0), ne1(ne1), r(r) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        // dst: the quantized weight buffer
        ggml_tensor * dst = ggml_new_tensor_2d(ctx, quant_type, ne0, ne1);
        ggml_set_name(dst, "dst");
        ...
        // Write f32 data into dst via SET_ROWS (includes WHT + quantize)
        ggml_tensor * written = ggml_set_rows(ctx, dst, src, row_idxs);
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
            if (t->type == GGML_TYPE_I64 || t->type == GGML_TYPE_I32) {
                if (ggml_is_view_op(t->op)) continue;
                init_set_rows_row_ids(t, ne1);
            } else if (t->type == quant_type) {
                // Zero-fill quantized dst to avoid fp16 NaN in unwritten rows' d0/d1
    ...
    double max_nmse_err() override {
        // GPU and CPU quantization diverge due to subgroupAdd reduction
        // order vs CPU serial addition in the iterative scale refinement. The
        // difference compounds over several iterations, leading to different
        // centroid selections and thus different dequanted values. Both are
        // valid quantizations of comparable quality.
        return 5.0;
    }
};
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // TQ3_1S/TQ4_1S: Gemma-4 E2B dimensions. The fused mul_mat_vec kernel has a
    ...
    for (ggml_type quant_type : {GGML_TYPE_TQ3_1S, GGML_TYPE_TQ4_1S}) {
        for (int k : { 1536, 2048, 2304, 3072, 4096 }) {
            for (int m : { 256, 1152, 1536, 2048, 5120, 6144 }) {
                for (int n : { 1, 2, 4, 8 }) {
                    test_cases.emplace_back(new test_mul_mat(quant_type, GGML_TYPE_F32, m, n, k, {1, 1}, {1, 1}));
                    test_cases.emplace_back(new test_mul_mat(quant_type, GGML_TYPE_F16, m, n, k, {1, 1}, {1, 1}));
                }
            }
        }

        // Large-batch MUL_MAT exercises the dequant + f16 matmul path used
        ...
        // are dequantized via pipeline_dequant[quant_type] into a temporary f16 buffer and
    ...
    // SET_ROWS with TQ3_1S/TQ4_1S destination: quantize then dequant round-trip
    for (ggml_type quant_type : {GGML_TYPE_TQ3_1S, GGML_TYPE_TQ4_1S}) {
        for (ggml_type idx_type : {GGML_TYPE_I32, GGML_TYPE_I64}) {
            for (int64_t ne0 : {32, 64, 128, 256}) {
                for (int r : {1, 4}) {
                    test_cases.emplace_back(new test_set_rows_tq4_1s(idx_type, ne0, 16, r, quant_type));
                }
            }
        }
        // Large tensor
        test_cases.emplace_back(new test_set_rows_tq4_1s(GGML_TYPE_I32, 128, 256, 64, quant_type));
    }
